...
// ===== CONVERSATIONS ENDPOINT (WhatsApp Chats) =====

// GET /api/conversations - Retorna todas as conversas WhatsApp do usuário

// GET /api/conversations/:id/messages - Retorna mensagens de uma conversa

// POST /api/conversations/:id/messages - Envia uma mensagem
app.post('/api/conversations/:id/messages', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { content, message_type = 'text' } = req.body;
    
    console.log(`📤 POST /api/conversations/${id}/messages - Enviando mensagem`);
    
    // Buscar informações do chat
    const chatResult = await pool.query(
      'SELECT chat_id, contact_number, instance_id FROM whatsapp_chats WHERE id = $1',
      [id]
    );
    ...
    // TODO: Enviar mensagem via WhatsApp (implementar integração com baileys)
    ...
    const messageResult = await pool.query(
      `INSERT INTO whatsapp_messages 
       ...
       VALUES 
        ($1, (SELECT company_id FROM whatsapp_instances WHERE id = $1), $2, 'system', $3, $4, $5, true, $6, NOW())
       RETURNING *`,
      [
        chat.instance_id,
        `msg_${Date.now()}`,
    ...
    await pool.query(
      `UPDATE whatsapp_chats 
       SET last_message = $1, last_message_at = NOW(), total_messages = total_messages + 1
       WHERE id = $2`,
      [content, id]
    );
...
app.get('/api/conversations', async (req: Request, res: Response) => {
  try {
    console.log('📋 GET /api/conversations - Buscando conversas WhatsApp');
    
    // Buscar primeira instância WhatsApp conectada
    const instancesResult = await pool.query(
      'SELECT id FROM whatsapp_instances WHERE status = $1 LIMIT 1',
      ['connected']
    );
    ...
    const conversations = chatsResult.rows.map(chat => ({
      id: chat.id,
      contact_id: chat.id,
      contact_name: chat.contact_name || chat.contact_phone,
    ...
    console.log(`✅ Retornando ${conversations.length} conversa(s)`);
    res.setHeader('Cache-Control', 'no-store, max-age=0');
    res.json({ conversations });
    
  } catch (error) {
    console.error('❌ Erro ao buscar conversas:', error);
    res.status(500).json({ error: 'Erro ao buscar conversas', conversations: [] });
  }
});
...
app.get('/api/conversations/:id/messages', async (req: Request, res: Response) => {
  try {
    ...
    console.log(`✅ ${result.rows.length} mensagem(ns)`);
    res.json({ messages: result.rows });
    
  } catch (error) {
    console.error('❌ Erro ao buscar mensagens:', error);
    res.status(500).json({ error: 'Erro ao buscar mensagens', messages: [] });
  }
});
...
app.post('/api/conversations/:id/messages', async (req: Request, res: Response) => {
  try {
    ...
    // Buscar chat
    const chatResult = await pool.query(
      'SELECT chat_id, contact_number, instance_id, company_id FROM whatsapp_chats WHERE id = $1',
      [id]
    );
